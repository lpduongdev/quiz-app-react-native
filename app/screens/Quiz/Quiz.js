...
import {buildURL, shuffle} from '../../utils/Utils';
import {data} from '../../data/QuizDataSample';
import SharedContext from '../../utils/Context';

const Quiz = () => {
  let [allQuestions, setAllQuestions] = useState(data);
  ...
  useLayoutEffect(() => {
    fetch(buildURL(props))
      .then(r => r.json())
      .then(res => {
        const result = res.map(question => {
          question.incorrectAnswers.push(question.correctAnswer);
          shuffle(question.incorrectAnswers);
          return {
          ...
          <Modal
            ...
            <View
              ...
              <View
                ...
                <Text
                  onPress={() => navigator.navigate('Menu')}
                  style={{
                    margin: 20,
                    textAlign: 'center',
                    color: COLORS.accent,
